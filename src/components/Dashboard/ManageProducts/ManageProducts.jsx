...
const ManageProducts = () => {
    ...
    const handleDeleteService = async (_id) => {
        try {
            const res = await axios.delete(`http://localhost:5000/product/delete/${_id}`, config)
            res && Swal.fire({
                icon: "success",
    ...
    return (
        <section className="mt-5 pt-5 shadow">
            <div data-aos="fade-up" className={style.titleText}>
                <h2 className={style.title}>Manage<span className={style.titleHalf}> Products</span></h2>
            </div>
